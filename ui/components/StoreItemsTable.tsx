// ui/components/StoreItemsTable.tsx
"use client";
...
export default function StoreItemsTable({
  ...
  /**
   * Reset local state whenever the store, query ou initialItems mudam.
   * Isto garante que mudar de loja ou de pesquisa não fica com lixo antigo.
   */
  useEffect(() => {
    ...
    setSortField("nome");
    setSortDir("asc");
  }, [storeId, query, initialItems, totalCount]);
  ...
  async function changeSort(nextField: SortField) {
    ...
    const { items: firstItems, count } = await fetchPage(
      0,
      pageSize,
      nextField,
      nextDir
    ...
    // Mesmo que o count não mude, é mais rigoroso atualizar offset com o length real
    setItems(firstItems);

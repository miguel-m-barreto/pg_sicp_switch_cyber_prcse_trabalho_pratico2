...
def _scrape_category_with_api_and_selenium(
    ...
    while True:
        ...
        print(f"[Auchan] Fetching page chunk: start={start}, sz={sz}")
        ...
        if not page_products:
            print("[Auchan] No products in chunk, stopping pagination.")
            break
        ...
        print(f"[Auchan] Chunk added {added} new products (total so far: {len(all_products)})")

        if added < sz:
            # last chunk
            print("[Auchan] Last chunk smaller than sz, reached end of results.")
            break
...
def scrape_auchan(produto: Optional[str] = None) -> List[Dict]:
    ...
    run_timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")  # safe filename
...
def _detect_categoria_from_url(url: str) -> Optional[str]:
    """Extract category slug from an Auchan URL like /pt/produtos-frescos/."""
    parsed = urlparse(url)
    if "auchan.pt" not in (parsed.netloc or ""):
        return None

    parts = [p for p in parsed.path.split("/") if p]
    # Expect something like ['pt', 'produtos-frescos', ...]
    if "pt" in parts:
        idx = parts.index("pt")
        if idx + 1 < len(parts):
            return parts[idx + 1]
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python3 scrapers/auchan.py \"categoria:produtos-frescos\"")
        print("  python3 scrapers/auchan.py \"arroz\"")
        print("  python3 scrapers/auchan.py \"https://www.auchan.pt/pt/produtos-frescos/\"")
        sys.exit(1)
    ...
    if arg.startswith("http"):
        categoria = _detect_categoria_from_url(arg)
        if not categoria:
            print("[Auchan] Could not detect category slug from URL, aborting.")
            sys.exit(1)

        print(f"[Auchan] Detected category slug from URL: {categoria}")
        produtos = scrape_auchan(f"categoria:{categoria}")
    else:
        # Direct query mode (categoria:..., plain search text, or empty)
